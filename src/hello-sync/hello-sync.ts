...
export class SyscallResponse {
  constructor(public id: number, public name: string, public args: any[]) {}
  ...
  static From(ev: MessageEvent): SyscallResponse {
    ...
    for (let i = 0; i < SyscallResponse.requiredOnData.length; i++) {
      if (!ev.data.hasOwnProperty(SyscallResponse.requiredOnData[i])) return;
    }
...
export class USyscalls {
  ...
  syscallAsync(cb: SyscallCallback, name: string, args: any[], transferrables?: any[]): void {
    if (!transferrables) transferrables = undefined;
  ...
  addEventListener(type: string, handler: SignalHandler): void {
    if (!handler) return;
    if (this.signalHandlers[type]) this.signalHandlers[type].push(handler);
    else this.signalHandlers[type] = [handler];
  }
  private resultHandler(ev: MessageEvent): void {
    ...
    if (response.name) {
      ...
      if (handlers) {
        for (let i = 0; i < handlers.length; i++) handlers[i](response);
      } else {
...
function init1(data: SyscallResponse): void {
  ...
  syscall.syscallAsync(personalityChanged, 'personality', [PER_BLOCKING, heap, waitOff], [heap]);
...
function main(): void {
  ...
  let s = 'hello, world!\n';
  let len = s.length;
  for (let i = 0; i < len; i++) heapu8[i] = s.charCodeAt(i);
