...
export function read(
  ...
  syscall.pread(fd, len, pos, function readFinished(
    err: any,
    lenRead: number,
    data: Uint8Array,
  ): void {
    ...
    for (let i = 0; i < lenRead; i++) buffer.writeUInt8(data[i], offset + i);
...
export function writeBuffers(fd: number, chunks: any, pos: number, req: FSReqWrap): void {
  ...
  function chunkComplete(err: any, count: number): void {
    ...
    written += count;
    if (done === chunks.length) req.complete(null, written);
  }
