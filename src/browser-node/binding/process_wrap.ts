...
export class Process {
  ...
  spawn(opts: SpawnOptions): any {
    ...
    for (let i = 0; i < opts.stdio.length; i++) {
      let f = opts.stdio[i];
      if (f.type !== 'fd') throw new Error('unsupported type ' + f.type + ' for FD ' + f.fd);
      files.push(f.fd);
    }
    ...
    if (cmds[0].indexOf('/') === -1) {
      cmds = bins.map((p: string) => {
        ...
        if (p.length && p[p.length - 1] === '/') p = p.slice(0, -1);
        return p + '/' + opts.file;
      });
    }
    ...
    let trySpawn = (): void => {
      ...
      syscall.spawn(cwd, cmd, opts.args, opts.envPairs, files, (err: any, pid: number) => {
        if (err === -ENOENT && cmds.length) {
          ...
          if (this.onexit) this.onexit(-128, -1);
          return;
        }
        this.pid = pid;
  ...
  handleSigchild(data: SyscallResponse): void {
    ...
    if (this.onexit) this.onexit(exitCode, signalCode);
  }
}
