...
function log(fmt: string, ...args: any[]): void {
  ...
  if (cb) process.stderr.write(msg, cb);
...
function parseArgs(args: string[], handlers: { [n: string]: Function }): [string[], boolean] {
  ...
  function usage(): void {
    errs++;
    let prog = process.argv[1].split('/').slice(-1);
    let flags = Object.keys(handlers)
      .concat(['h'])
      .sort()
      .join('');
  ...
  if (!args.length) {
    ...
    args.map(function(path: string, i: number): void {
      if (path === '-') {
        ...
        // stdout.
        if (++opened === args.length) setTimeout(concat, 0, files, process.stdout, code);
        return;
      }
